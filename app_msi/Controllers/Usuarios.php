...
print_r($nivel);
    log_message("{$nivel}"," +++++++++++++++++++++++++++++++ ");
    log_message("{$nivel}"," Config Nivel Error ");
    log_message("{$nivel}", "{print_r(env('database.default.username'))}");
    log_message("{$nivel}"," +++++++++++++++++++++++++++++++ ");
    // Obtenemos el nombre del Controlador/Metodo
        $router = \Config\Services::router();
        $_method = $router->methodName();
        ...
        $controlador = explode('\\', $_controller) ;
        $this->clase = $controlador[max(array_keys($controlador))] ;       
        log_message("{$nivel}", "Nivel Threshold {$config->threshold} - {$nivel}");   
        log_message("{$nivel}"," ----------------------------");
        log_message("{$nivel}", 'Controlador=>' . $this->clase . '/' . $router->methodName() );
        log_message("{$nivel}"," ----------------------------");
